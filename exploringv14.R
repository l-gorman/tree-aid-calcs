...
ntfp_sold_and_consumed_calculation <- function(
        ...
        fp_props_sold_numeric, # should this be changed to prop for consistency?
        fp_amount_sold_kg,

        fp_prop_consumed_numeric,
        fp_prop_consumed_kg,

        fp_props_processed_numeric,
        fp_amount_processed_kg,

        fp_props_process_sold_numeric, # e.g. "roots_process_sold_prop_numeric" # new
        fp_prop_process_consumed_numeric # e.g. "roots_process_eaten_prop_numeric" # new
) {
    # Beginning with ntfp sold kg TOTAL
    ...
    processed_sold_prop_data <- data[processed_sold_columns] # new

    amount_sold_kg <- tibble::as_tibble((harvest_data * sold_prop_data) + (harvest_data * processed_sold_prop_data)) # new
    ...
    # Moving on to NTFP consumed TOTAL
    ...
    if (all(harvested_columns %in% colnames(data)) == T & all(consumed_columns %in% colnames(data)) == T) {
        ...
        processed_consumed_prop_data <- data[processed_consumed_columns] # new

        amount_consumed_kg <- tibble::as_tibble((harvest_data * consumed_prop_data) + (harvest_data * processed_consumed_prop_data)) # new
    ...
    number_of_loops <- find_number_of_loops(tree_aid_df, name_column = "fp_name")
    harvested_columns <- paste0(fp_harvest_kg, "_", c(1:number_of_loops))
    PROCESSED <- paste0(fp_props_processed_numeric, "_", c(1:number_of_loops))




    # NTFP PROCESSED CONSUMED KG

    processed_kg_columns <- paste0(fp_processed_kg, "_", c(1:number_of_loops))
    processed_consumed_columns <- paste0(fp_prop_process_consumed_numeric, "_", c(1:number_of_loops)) # new


    # NTFP PROCESSED SOLD KG
    processed_sold_columns <- paste0(fp_props_process_sold_numeric, "_", c(1:number_of_loops)) # new
...
tree_aid_df <- ntfp_sold_and_consumed_calculation(
  ...
  fp_prop_consumed_kg="leaves_amount_consumed_kg",
...
#                                  TO DO
# 1) PROPORTIONS PROCESSED
# 2) SUM PROCESSED COLUMNS WITH NON- PROCESSED AT END CALCULATIONS STAGE?
# 3) SHEA BUTTER
# 4) HONEY
